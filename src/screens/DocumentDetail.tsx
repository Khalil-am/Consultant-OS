              ...
              {tasks.length > 0 && (
                <div className="section-card">
                  <div className="section-card-header">
                    ...
                    <button className="btn-ghost" style={{ fontSize: '0.72rem', height: '26px', padding: '0 0.625rem' }} onClick={() => setActiveTab('Tasks')} aria-label="View all linked tasks">
            ...
            {tasks.length === 0 ? (
              ...
              <div>
                {(taskSort === 'title'
                  ...
                  : taskSort === 'priority'
                  ? [...tasks].sort((a, b) => { const ord = { High: 0, Medium: 1, Low: 2 }; return (ord[a.priority as keyof typeof ord] ?? 3) - (ord[b.priority as keyof typeof ord] ?? 3); })
                  : tasks
                ).filter(t => (taskPriorityFilter === 'All' || t.priority === taskPriorityFilter) && (!taskSearch.trim() || t.title.toLowerCase().includes(taskSearch.toLowerCase()))).map((task, i, arr) => (
                  <div key={task.id} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.875rem 1.25rem', borderBottom: i < arr.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                    <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: task.priority === 'High' ? '#EF4444' : task.priority === 'Medium' ? '#F59E0B' : '#10B981', flexShrink: 0 }} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  {messages.length > 1 && (
                    <button
                      ...
                      onClick={() => setMessages([{
                        role: 'assistant',
                        content: `Chat cleared. I'm ready to help you with **${doc.name}**. What would you like to know?`,
                      }])}
                      style={{ background: 'none', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '4px', cursor: 'pointer', color: '#475569', fontSize: '0.68rem', padding: '2px 8px', fontFamily: 'inherit' }}
                    >
                      Clear
                    </button>
                  )}
